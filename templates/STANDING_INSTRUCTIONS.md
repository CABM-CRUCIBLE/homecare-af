...
* **Clean Architecture:** Structure the backend into layers: `Domain` (Entities), `Application` (Interfaces/Logic/CQRS), `Infrastructure` (DB/External Services/EF Core), and `API` (Controllers).
...
* **Typing:** No `any` types in TypeScript. Use strict interfaces and types.

## 3. Security-First Implementation
* **Secrets Management:** Never hardcode keys. Use `appsettings.json` (and `UserSecrets` for dev) patterns.
* **Input Validation:** Use FluentValidation on the backend and Zod on the frontend.
* **Multi-Tenancy:** Strictly enforce tenant isolation in all queries using tenant filter expressions or tenant-scoped DbContext query filters.
* **Exception Handling:** Implement a Global Exception Handling Middleware returning RFC 7807 (Problem Details). Do not expose stack traces in Production.
...
* **Test Coverage:** Aim for high coverage on Business Logic services, handlers, and domain rules.
...
* **State Management:** Restrict global client state strictly to UI state (like theme) or authentication context.
* **Hooks:** Extract reusable client-side logic into Custom Hooks.
