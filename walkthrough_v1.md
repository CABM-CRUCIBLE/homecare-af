...
### Phase 11: Multi-Tier & Per-Node LLM Model Routing (New)
- [config.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/config.py): Added `model_architecture`, `model_code`, `model_review`, `model_intake`, and granular `node_models` dictionary with fallback resolution in `get_model_for_node()`.
- [provider.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/llm/provider.py): Built dynamic LLM caching (`_model_cache`) and automatic node-level routing via `ainvoke(..., node_name=...)`.
- [state.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/state.py): Added model routing fields to `AgentState` for run-time override capability.
- **Node Integration**: Wired `node_name` and `state_overrides` through [intake.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/intake.py), [analyze.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/analyze.py), [architect.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/architect.py), [generate_prompts.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/generate_prompts.py), [execute_prompt.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/execute_prompt.py), [review_arch.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/review_arch.py), and [code_review.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/graph/nodes/code_review.py).
- **CLI & Web UI**:
  - [main.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/main.py): Added `--model-arch`, `--model-code`, and `--model-review` options to `homecare-agent run`.
  - [web.py](file:///c:/WorkingFolder/homecare-af/src/homecare_agent/ui/web.py): Added Multi-Tier Model Configuration accordions and selectors.
- [test_model_routing.py](file:///c:/WorkingFolder/homecare-af/tests/test_model_routing.py): Comprehensive test suite verifying tier fallbacks, granular node overrides, state overrides, and client caching.
...
- `tests/test_model_routing.py`: 5 passed (NEW)
...
```
homecare-agent run --help
 Options:
   --name          -n    TEXT  Feature name
   --desc          -d    TEXT  Feature description
   --repo          -r    TEXT  Path to target repository
   --wireframe     -w    TEXT  Wireframe paths or URLs
   --model         -m    TEXT  Default OpenRouter model
   --model-arch          TEXT  High-reasoning model for Architecture Analysis & Strategy (e.g., Claude Sonnet/Opus)
   --model-code          TEXT  Cost-effective model for Work Package code generation (e.g., DeepSeek Coder, Haiku)
   --model-review        TEXT  Model for Architecture and PR Code Reviews
   --interactive         TEXT  Interactive mode [default: interactive]
   --log-level     -l    TEXT  Logging level [default: INFO]
```
All commands and multi-tier model options are registered and functional.
